...
module.exports.tapMedicine = async (req,res) => {
    let response = {...constants.defaultServerResponse};
    try {
        const responseFromService = await medicineService.tapMedicine(req.body);
        response.status = 200;
        response.message = constants.medicineMessage.ADD_SUCCESS;
        response.body = responseFromService;
    } catch (error) {
        console.log('Something wrong: Controller: tapMedicine', error);
        response.message = error.message;
    }
    return res.status(response.status).send(response);
}

module.exports.getQtdeByName = async (req,res) => {
    let response = {...constants.defaultServerResponse};
    try {
        const responseFromService = await medicineService.getQtdeByName(req.body);
        response.status = 200;
        response.message = constants.medicineMessage.ADD_SUCCESS;
        response.body = responseFromService;
    } catch (error) {
        console.log('Something wrong: Controller: getQtdeByName', error);
        response.message = error.message;
    }
    return res.status(response.status).send(response);
}